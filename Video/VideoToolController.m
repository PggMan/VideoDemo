...
#import "VideoToolController.h"
#import <AVFoundation/AVFoundation.h>
@interface VideoToolController ()<AVCaptureVideoDataOutputSampleBufferDelegate>
@property (nonatomic ,weak)  AVCaptureSession *session;
@property (nonatomic ,weak)  AVCaptureVideoPreviewLayer *previewLayer;
@property (nonatomic ,weak) UIImageView *imageView;
@end

@implementation VideoToolController
...
- (void)addCaptureDeviceOutput:(AVCaptureDevice *)device{
    ...
    [output setSampleBufferDelegate:self
                              queue:dispatch_queue_create("LinXunFengSerialQueue", DISPATCH_QUEUE_SERIAL)];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    for (AVMetadataMachineReadableCodeObject *metadata in metadataObjects) {
        if ([metadata.type isEqualToString:AVMetadataObjectTypeQRCode]) {
            ...
            if ([metadata.stringValue length] >0)
            {
